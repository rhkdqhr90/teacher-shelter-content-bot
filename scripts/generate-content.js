/**
 * 교사쉼터 자동 콘텐츠 생성 스크립트 (v2 - 승인 방식)
 *
 * 흐름:
 * 1. 봇 계정으로 로그인 → accessToken 발급
 * 2. Claude API (웹검색) → 콘텐츠 3개 생성
 * 3. 교사쉼터 API에 DRAFT 상태로 저장
 * 4. 관리자가 어드민에서 확인 후 승인 → 공개
 */
...
async function login() {
  ...
  // 응답 구조에 맞게 조정 필요 (accessToken 위치)
  const token = data.accessToken || data.access_token || data.token;

  if (!token) {
    throw new Error("로그인 응답에 accessToken 없음: " + JSON.stringify(data));
  }
...
async function main() {
  ...
  if (!API_URL || !BOT_PASSWORD) {
    console.log("⚠️  API_URL 또는 BOT_PASSWORD 없음. 로컬 테스트 모드.");
  }
